...
Проект настроен на автоматическое резервное копирование через cron-задания. По умолчанию резервные копии создаются ежедневно в 3:00 и хранятся в течение 7 дней.
...
Эта команда создает резервные копии следующих компонентов:
- База данных PostgreSQL (для n8n и Zep)
- Конфигурация workflows n8n
- Данные Weaviate
- Модуль памяти Zep

Резервные копии сохраняются в каталоге `/opt/backups/n8n-super-agent` на сервере. Рекомендуется периодически копировать эти резервные копии во внешнее хранилище.

### Восстановление из резервной копии

```bash
make restore BACKUP_FILE=/opt/backups/n8n-super-agent/backup-YYYYMMDD-HHMMSS.tar.gz
```

Эта команда восстанавливает все данные из указанного файла резервной копии.

## Безопасность

Проект n8n-super-agent включает ряд механизмов для повышения безопасности. Ниже приведены рекомендации и опции настройки:

### Сетевая безопасность

1. **Hetzner Cloud Firewall**
   - Ограничьте доступ по SSH только с ваших IP-адресов. Измените параметр `hetzner_allow_ssh_from_ips` в файле `ansible/group_vars/n8n_servers/vars.yml`.
   - Для усиления защиты разрешите доступ по HTTP/HTTPS только с IP-адресов Cloudflare, установив `cloudflare_restrict_direct_access: true`.

2. **Cloudflare WAF**
   - Проект настроен на использование улучшенных правил WAF Cloudflare:
     - Блокировка сканеров уязвимостей
     - Блокировка запросов с пустым User-Agent
     - Защита чувствительных путей администрирования
     - Возможность блокировки запросов из определенных стран

### Безопасность SSH

1. **Аутентификация по ключам**
   - Раскомментируйте и настройте параметр `ssh_public_key` в `ansible/group_vars/n8n_servers/vars.yml` для использования вашего публичного ключа

2. **Fail2Ban**
   - Проект автоматически настраивает Fail2Ban для защиты от брутфорс-атак
   - Можно настроить параметры в `ansible/group_vars/n8n_servers/vars.yml`

### Автоматические обновления безопасности

Проект настраивает `unattended-upgrades` для автоматической установки обновлений безопасности Ubuntu. Настройки можно изменить в `ansible/group_vars/n8n_servers/vars.yml`.
